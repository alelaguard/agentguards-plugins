---
name: setup
description: Set up and verify AgentGuards in GitHub Copilot CLI against a self-hosted appliance. Use when the user asks to configure AgentGuards, set their URL/API key, or check that the guardrails are wired up correctly.
---
...
1. **Check for `AGENTGUARDS_URL`.** This is required here (the hosted plugin
   ...
   from wherever Copilot CLI runs, over HTTPS.
...
5. **Restart Copilot CLI** so it inherits them.
