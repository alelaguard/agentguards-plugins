---
name: status
description: Report AgentGuards guardrail status. Use when the user asks whether AgentGuards is active, healthy, or correctly configured in Gemini CLI.
---
...
3. If the service is unreachable or the key is missing, walk the user through the
   setup steps (set `AGENTGUARDS_API_KEY` and restart Gemini CLI).
