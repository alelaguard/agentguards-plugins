---
name: setup
description: Set up and verify AgentGuards in Codex against a self-hosted appliance. Use when the user asks to configure AgentGuards, set their URL/API key, or check that the guardrails are wired up correctly.
---
...
1. **Check for `AGENTGUARDS_URL`.** This is required here (the hosted plugin
   ...
   to the wrong instance). It should be the appliance's own address, reachable
   from wherever Codex runs, over HTTPS.

2. **Check for the API key.** Look for `AGENTGUARDS_API_KEY`, or a saved token
   at `~/.codex/agentguards_token`. If neither exists, tell the user to
   generate one on their appliance's own console at
   `<AGENTGUARDS_URL>/admin/ui/keys`, then either:
   ```
   echo "ag_..." > ~/.codex/agentguards_token
   ```
   or export `AGENTGUARDS_API_KEY` instead.
...
4. **Tell the user to add `AGENTGUARDS_URL` (and `AGENTGUARDS_API_KEY`, if used
   instead of the token file) to their shell profile** (`~/.bashrc`,
   `~/.zshrc`, …), not just export them in the current terminal — a one-off
   `export` is gone the moment the terminal closes or a new agent session
   starts.

5. **Register the hooks** in `~/.codex/config.toml` if not already present —
